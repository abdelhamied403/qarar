...
class ArchivedDrafts extends Component {
  constructor(props) {
    super(props);
  ...
  getTags = async () => {
    const tagsResponse = await Api.get(
      `/qarar_api/load/vocabulary/tags?_format=json&status=archived`
    );
    if (tagsResponse.ok) {
    ...
    return (
      <div className="drafts">
        <div className="draftHeader">
          <Container>
            <h3>{translate('archivedDraftsPage.title')}</h3>
          </Container>
        </div>
        <Container>
                ...
                <Col xs="12" md="4" className="filter-buttons">
                  <div className="form-group">
                    ...
                    <ReactSelect
                      ...
                      loadingMessage={() =>
                        translate('archivedDraftsPage.keywordsLoadingMessage')
                      }
                      cl
                      onChange={selected =>
            ...
            {drafts.length ? (
              drafts.map(draft => (
                <CardDraft
                  ...
                  subHeader={`${translate(
                    'archivedDraftsPage.draftCard.votingCloses'
                  )}${draft.end_date}`}
                  content={draft.body}
                  votes={
              ...
              <Col className="text-start">
                <Alert type="sucess">
                  {translate('archivedDraftsPage.noDecisions')}
                  <Link href="/decisions">
                    <a>{translate('archivedDraftsPage.archivedDecisions')}</a>
                  </Link>
                </Alert>
              </Col>
            )}
...
export default connect(mapStateToProps)(ArchivedDrafts);
